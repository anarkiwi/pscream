/* Primal Scream 1.0 prime number generator.
   ...
   Updated by Ewen McNeill <ewen@naos.co.nz>, 1999/8/28, for new stack ops.
...
sieve *initsievenode(const cardinal multiple, const cardinal prime) {
  ...
  if ((newnode = (sieve*)malloc(sizeof(sieve)))) {             /* MALLOC */
...
sieve **searchsieve(const cardinal key) {
  register sieve **parent, *curr = root;

  do {
/* Go to lesser or greater if our key is lesser or greater than the key of 
   the current node. */
/*    parent = (key < curr->multiple) ? &(curr->lesser) : &(curr->greater);  */
...
/* Keep going, as long as the current node isn't empty, and isn't the one
   we're looking for. */
  } while ((curr = *parent) ? (curr->multiple != key) : 0);
...
static void prewalk(sieve* (*visit)(sieve **parent)) {
  cardinal sievemembers = 0;
  prestacktype *stack = initprestack(&(root->greater));
  sieve *curr, **parent;
...
static void postwalk(void (*visit)(sieve *curr)) {
  cardinal sievemembers = 0, totalheight = 0, branches = 0, height = 0;
  poststacktype *stack = initpoststack(root->greater);
  sieve *curr;
...
/* Initialise the sieve structure with the DummyKey value. Used to make a
   ...
   stuff. */
void initsieve(void) {
  root = initsievenode(DUMMY_KEY, DUMMY_KEY);
...
/* Free up the tree, then free the root. */
void donesieve(void) {
  postwalk(donenode);
  free(root);
